...
CMD="mpirun --hostfile \$OAR_NODEFILE -n NUMBERTHREADS --mca btl_tcp_if_include bond0 tests.py"

# Specify the range of thread counts you want to test
THREAD_COUNTS=(4 8 16 32 33 64 65 96)
...
for NUM_THREADS in "${THREAD_COUNTS[@]}"; do
    ...
    echo "run started with $NUM_THREADS" 
    # Run the command and capture the output
    RESULT=$(eval "$CURRENT_CMD")
    echo "run stoped with $NUM_THREADS"
