...
if __name__ == "__main__":
    ...
    for i in range(num_executions):
        ...
        if rank == 0:
            ...
            ts = time.time()
            search_array_parallel(arr, len(arr), checkFunction=fun, transformFunc = lambda x: 10*x if x !=None else None )
            ts = time.time() - ts
            execution_time = ts
            execution_times.append(execution_time)
            print("ended execution",i,"with time",execution_time)
        else:
            search_array_parallel(None, len(arr), checkFunction=fun, transformFunc = lambda x: 10*x if x !=None else None )
    ...
    if rank == 0:
        ...
        data = np.column_stack((comm.Get_size(), avg_execution_time))
        np.savetxt('execution_times_mpi_'+ str(comm.Get_size()) +'.txt', data, delimiter=',', header='Nodes,Execution Time (s)', fmt='%d,%.6f')
'''
